...
class RobotController:
    ...
    def jog_cartesian(self, robot, direction, step_size=0.005, num_steps=50, frame='base'):
        """
        Jog robot in a Cartesian direction using RMRC with improved accuracy.
        
        Args:
            robot: Robot to jog
            direction: 'x+', 'x-', 'y+', 'y-', 'z+', 'z-'
            step_size: Distance to move (meters)
            num_steps: Number of RMRC steps (more = more accurate but slower)
            frame: 'base' for world coordinates
        ...
        start_position = current_pose.t.copy()
        
        # Create increment transform
        increment = SE3()  # Identity
        
        if direction == 'x+':
            increment = SE3.Tx(step_size)
        elif direction == 'x-':
            increment = SE3.Tx(-step_size)
        elif direction == 'y+':
            increment = SE3.Ty(step_size)
        elif direction == 'y-':
            increment = SE3.Ty(-step_size)
        elif direction == 'z+':
            increment = SE3.Tz(step_size)
        elif direction == 'z-':
            increment = SE3.Tz(-step_size)
        else:
        ...
        # Apply transformation (base frame: pre-multiply)
        target_pose = increment * current_pose
        
        # Use RMRC to move to target
        print(f"[{robot.name}] Jogging {direction} by {step_size*1000:.1f}mm")
        success, final_q = self.move_rmrc(robot, target_pose, num_steps)
        
        if success:
            # Verify actual movement
            final_pose = robot.fkine(robot.q)
            actual_movement = final_pose.t - start_position
            axis_idx = 0 if 'x' in direction else (1 if 'y' in direction else 2)
            actual_distance = abs(actual_movement[axis_idx]) * 1000  # Convert to mm
